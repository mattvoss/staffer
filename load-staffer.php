	...
	<div class="wrap">
		...
		<form method="post" action="options.php">
			...
			<table class="form-table">
				<tr valign="top">
					<th scope="row"><?php _e( 'Staff Layout', 'staffer' ); ?></th>
					<td><input name="staffer[gridlayout]" type="checkbox"
							value="1" <?php checked( true, $stafferoptions['gridlayout'] ); ?> />
				...
				<tr valign="top">
					<th scope="row"><?php _e( 'Manual Mode', 'staffer' ); ?></th>
					<td><input name="staffer[manual_mode]" type="checkbox"
							value="1" <?php checked( true, $stafferoptions['manual_mode'] ); ?> />
				...
				<tr valign="top">
					<th scope="row"><?php _e( 'Excerpt Style', 'staffer' ); ?></th>
					<td>
						<?php $staffer_estyle = $stafferoptions['estyle']; ?>
						<select name="staffer[estyle]">
							<option value="excerpt" <?php if ( $staffer_estyle == "excerpt" ) {
								echo( 'selected="selected"' );
							} ?> ><?php _e( 'Excerpt', 'staffer' ); ?></option>
							<option value="full" <?php if ( $staffer_estyle == "full" ) {
								echo( 'selected="selected"' );
							} ?> ><?php _e( 'Full', 'staffer' ); ?></option>
							<option value="none" <?php if ( $staffer_estyle == "none" ) {
								echo( 'selected="selected"' );
							} ?> ><?php _e( 'Disabled', 'staffer' ); ?></option>
						</select>
				...
				<tr valign="top">
					<th scope="row"><?php _e( 'Staffer Main Page Description', 'staffer' ); ?></th>
					<td>
						<?php
						$text = $stafferoptions['sdesc'];
						wp_editor( $text, 'sdesc', array(
							'textarea_name' => 'sdesc',
							'media_buttons' => true,
							'textarea_rows' => '10'
						) ); ?>
				...
				<tr valign="top">
					<th scope="row"><?php _e( 'Listings per page', 'staffer' ); ?></th>
					<td><input type="number" size="80" max="99" name="staffer[perpage]"
							value='<?php echo $stafferoptions['perpage']; ?>'/>
				...
				<tr valign="top">
					<th scope="row"><?php _e( 'Enable Sync with WordPress User Profile', 'staffer' ); ?></th>
					<td>
						<input name="staffer[wp_profile_sync]" type="checkbox"
							value="1" <?php checked( true, $stafferoptions['wp_profile_sync'] ); ?> />

						<p class="description"><?php _e( 'Check to enable sync staff member\'s data into WordPress user profile.<br /><code>Caution:</code> This will irreversibly update respective WordPress user\'s data.', 'staffer' ); ?></p>
					</td>
				</tr>

				<tr valign="top">
					<th scope="row"><?php _e( 'Override Author Link with Staffer', 'staffer' ); ?></th>
					<td>
						<input name="staffer[override_author_link]" type="checkbox"
							value="1" <?php checked( true, $stafferoptions['override_author_link'] ); ?> />

						<p class="description"><?php _e( 'Check to override the default author link with the staff member link.', 'staffer' ); ?></p>
					</td>
				</tr>

				<tr valign="top">
					<th scope="row"><?php _e( 'Display Synced Author\'s Posts in Staffer Profile', 'staffer' ); ?></th>
					<td>
						<input name="staffer[display_author_posts]" type="checkbox"
							value="1" <?php checked( true, $stafferoptions['display_author_posts'] ); ?> />

						<p class="description"><?php _e( 'Check to display synced author\'s posts under the staffer team member\'s profile page.', 'staffer' ); ?></p>
					</td>
				</tr>

				<tr valign="top">
					<th scope="row"><?php _e( 'Staffer Page Title', 'staffer' ); ?></th>
					<td><input type="text" size="80" name="staffer[ptitle]"
							value='<?php echo $stafferoptions['ptitle']; ?>'/>
				...
				<tr valign="top">
					<th scope="row"><?php _e( 'Staffer Label', 'staffer' ); ?></th>
					<td><input type="text" size="80" name="staffer[label]"
							value='<?php if ( isset ( $stafferoptions['label'] ) ) {
								echo $stafferoptions['label'];
							} ?>'/>
				...
				<tr valign="top">
					<th scope="row"><?php _e( 'Staffer URL slug', 'staffer' ); ?></th>
					<td><input type="text" size="80" name="staffer[slug]"
							value='<?php echo $stafferoptions['slug']; ?>'/>
				...
				<tr valign="top">
					<th scope="row"><?php _e( 'Disable CSS', 'staffer' ); ?></th>
					<td><input name="staffer[disablecss]" type="checkbox"
							value="1" <?php checked( true, $stafferoptions['disablecss'] ); ?> />

						<p class="description"><?php _e( 'Check this box to disable all Staffer CSS.', 'staffer' ); ?></p>
					</td>
				</tr>
				<tr valign="top">
					<th scope="row"><?php _e( 'Use custom wrappers', 'staffer' ); ?></th>
					<td><input name="staffer[customwrapper]" type="checkbox"
							value="1" <?php checked( true, $stafferoptions['customwrapper'] ); ?> />
				...
				<tr valign="top">
					<th scope="row"><?php _e( 'Custom start wrapper', 'staffer' ); ?></th>
					<td><input type="text" size="80" name="staffer[startwrapper]"
							value='<?php echo $stafferoptions['startwrapper']; ?>'/>

						<p class="description"><?php _e( 'Enter your custom start wrapper.', 'staffer' ); ?></p>
					</td>
				</tr>
				<tr valign="top">
					<th scope="row"><?php _e( 'Custom end wrapper', 'staffer' ); ?></th>
					<td><input type="text" size="80" name="staffer[endwrapper]"
							value='<?php echo $stafferoptions['endwrapper']; ?>'/>
				...
				<tr valign="top">
					...
					<td>
						<textarea rows="20" class="large-text" type="text" name="staffer[customcss]" cols="50"
							rows="10"/><?php echo $stafferoptions['customcss']; ?></textarea>
...
function staffer_validate( $input ) {
	...
	if ( ! isset ( $input['wp_profile_sync'] ) ) {
		$input['wp_profile_sync'] = null;
	}
	if ( ! isset ( $input['wp_profile_create'] ) ) {
		$input['wp_profile_create'] = null;
	}
	if ( ! isset ( $input['override_author_link'] ) ) {
		$input['override_author_link'] = null;
	}
...
function staffer_staff_role_box( $post ) {
	...
	echo '<label for="staffer_synced_wp_profile"><strong>WordPress User Profile to Sync With:</strong></label>';
	echo wp_dropdown_users( $args );
	echo '<br /><label for="staffer_synced_wp_profile"><em>If you don\'t find the user, its most probable that the user doesn\'t exist. Create an user to sync with.</em></label>';
}
